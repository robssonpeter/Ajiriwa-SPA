<template>
    <employer-layout title="Add Job">
        <div class="grid grid-cols-3 gap-3">
            <div class="z-40 px-4 min-h-screen col-span-2 bg-gray-50 text-gray-700 my-4 p-4 shadow-md description"
                v-html="$page.props.job.description">
            ...
            <div class="col-span-1">
                <div class="sticky top-20 shadow-md p-4 flex flex-col text-gray-700">
                    ...
                    <span><strong>Applications:</strong>
                        <Link :class="$page.props.job.applications_count ? 'text-green-500 font-bold' : ''"
                            :href="route('company.job.applications', $page.props.job.slug)">{{
                                $page.props.job.applications_count }}</Link>
                    </span>
                    <div>
                        <button @click="getRecommendedCandidates()"
                            class="bg-green-500 text-white p-2 font-bold hover:bg-green-600 mt-2 rounded-md">Show
                            Recommended Candidates</button>
                    </div>
                    <div class="mt-4">
                        <Link class="bg-gray-500 text-white p-2 font-bold hover:bg-green-600 mt-4 rounded-md"
                            :href="route('company.post-job')">Post Another Job</Link>
                    </div>
                </div>
            </div>
        </div>
        <dialog-modal :show="candidates_modal" closeable="true" :max-width="candidates_modal_size">
            ...
            <template v-slot:content>
                <section v-if="candidate"
                    class="hidden md:block md:col-span-2 lg:col-span-4 2xl:col-span-3 shadow-md pt-4 p-2 self-auto overflow-y-auto">
                    <button class="text-green-500  flex space-x-1 ml-8 hover:text-gray-700" title="Back to Candidates"
                        @click="candidate = false; candidates_modal_size = 'md'">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-3 h-3 self-center">
                ...
                <div class="text-gray-500" v-else>
                    ...
                    <div role="status" v-if="loading_candidates" v-for="x in 5"
                        class="mb-2 space-y-8 animate-pulse md:space-y-0 md:space-x-8 md:flex md:items-center">
                        <div
                            class="flex justify-center items-center h-24 bg-gray-300 rounded sm:w-48 md:w-32 dark:bg-gray-700">
                    ...
                    <div class="my-1 grid grid-cols-3 border border-gray-300 sm:rounded-md"
                        v-for="candidate in recommended_candidates">
                        <img :src="'https://placeimg.com/150/150/any?1'" class="h-24 rounded-l-md" alt="">
                        <div class="col-span-2 px-2">
                            <a href="#" class="text-green-500 font-bold md:text-xl" @click="viewCandidate(candidate.id)">{{
            ...
            <template v-slot:footer>
                <a v-if="candidate" :href="route('cv.print', candidate.id)"
                    class="cursor-pointer bg-green-500 hover:bg-green-600 text-white font-bold p-1 rounded-md"
                    @click="candidates_modal_size = '4xl'">Download CV</a>
            </template>
