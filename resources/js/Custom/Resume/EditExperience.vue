<template>
    <div>
        <section class=" gap-2">
            <div class=" border-dotted border border-gray-300 text-center flex gap-3 px-2 py-8 self-center"
            ...
            <transition v-for="(experience, index) in experiences" :key="experience" type="fade" mode="out-in" appear
                ...
                <div class="article-details border-gray-300 border p-2 my-1">
                    <div class="edit-experience grid sm:grid-cols-2 md:grid-cols-6 gap-2" v-if="experience.editing">
                        <div class="md:col-span-2 flex flex-col">
                            ...
                            <input type="text" v-model="experiences[index].title" class="focus:border-green-400 border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-green-400"
                                :id="'job_title_' + index">
                        </div>
                        <div class="md:col-span-2 flex flex-col">
                            <label :for="'company_' + index" class="font-bold">Company</label>
                            <input type="text" v-model="experiences[index].company" class="focus:border-green-400 border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-green-400"
                                :id="'job_company_' + index">
                        </div>
                        <div class="md:col-span-2 flex flex-col">
                            <label :for="'industry_' + index" class="font-bold">Industry</label>
                            <select class="focus:border-green-400 border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-green-400" v-model="experiences[index].industry_id"
                                :id="'industry_' + index">
                        ...
                        <div class="md:col-span-2 flex flex-col">
                            ...
                            <select class="focus:border-green-400 border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-green-400" v-model="experiences[index].country_id"
                                :id="'country_' + index">
                                <option value="">Select Country</option>
                                <option :value="country.iso_3166_1_alpha2" v-for="country in countries">{{ country.name }}
                                </option>
                            </select>
                        </div>
                        <div class="md:col-span-2 flex flex-col">
                            <label :for="'start_date_' + index" class="font-bold">Start Date</label>
                            <input type="date" v-model="experiences[index].start_date" class="focus:border-green-400 border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-green-400"
                                :id="'start_date_' + index">
                        </div>
                        <div class="md:col-span-2 flex flex-col">
                            <label :for="'end_date_' + index" class="font-bold">End Date</label>
                            <div class="flex flex-row gap-2">
                                <input v-if="experiences[index].currently_working" v-model="experiences[index].end_date"
                                    type="date" :disabled="experiences[index].currently_working"
                                    class="focus:border-green-400 border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-green-400 disabled:bg-gray-200" :id="'end_date_' + index">
                                <input v-else type="date" v-model="experiences[index].end_date"
                                    class="focus:border-green-400 border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-green-400 disabled:bg-gray-200" :id="'end_date_' + index">
                                <span class="self-center text-green-400 font-bold">Working</span>
                                <input @change="currently_workingControl(index)"
                                    :checked="experiences[index].currently_working"
                                    class="self-center text-green-500 focus:border-green-500 focus:border" type="checkbox">
                            </div>
                        </div>

                        <div class="md:col-span-6 flex flex-col">
                            <label :for="'responsibilities_' + index" class="font-bold">Responsibilities</label>
                            <textarea v-model="experiences[index].description" class="focus:border-green-400 border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-green-400"
                                :id="'responsibilities_' + index"></textarea>
                        </div>
                        <div class="flex flex-row gap-2">
                            <button @click="cancelEdit(index)"
                                class="rounded-md bg-gray-400 hover:shadow-lg text-white p-2 self-center">Cancel</button>
                            <button :disabled="false" @click="addExperience(index)"
                                class="bg-green-500 hover:bg-green-600 text-white font-semibold px-4 py-2 rounded-md self-center">
                    ...
                    <section v-else class="view-experience">
                        ...
                        <span class="text-muted italic">{{ experience.currently_working ? 'Present' :
                            experience.end_date_formatted }}</span>
                        ...
                        <div class="flex flex-row">
                            ...
                            <div class="article-cta candidate-experience-edit-delete flex">
                                <a href="javascript:void(0)" @click.prevent="editExperience(index)"
                                    class="btn btn-warning action-btn edit-experience text-green-500" data-id="2">
                                ...
                                <a href="javascript:void(0)" @click="remove(index)"
                                    class="btn btn-danger action-btn delete-experience text-red-500" data-id="2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="border-dotted border border-gray-300 text-center flex gap-3 justify-right px-2 py-4 self-center"
                v-if="experiences.length && allow_add">
                <span @click="newExperience"
                    class="rounded-md py-2 px-1 cursor-pointer border border-green-500 text-green-500 hover:text-white hover:bg-green-500 self-center">
